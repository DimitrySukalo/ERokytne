...
public static class SaveSupportMessageCommandHandlerTests
{
    [Fact]
    private static async Task HandlerMethodThrowsException()
    {
        ...
        var commandHandler = new SaveSupportMessageCommandHandler(telegramBot.Object, dbContext.Object, actionsService.Object);
        
        //Act and Assert
        await Assert.ThrowsAsync<ArgumentNullException>(async () => await 
            commandHandler.Handle(new SaveSupportMessageCommand
            {
                ChatId = "2",
    ...
    private static void HandlerMethodReturnsSuccessful()
    {
        ...
        actionsService.Setup(e => 
            e.DeleteUserCacheAsync(It.IsAny<string>())).Returns(Task.CompletedTask);
